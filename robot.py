...
class robot:
    def __init__(self):
        ...
        # self.device = BleakClient(self.address)


        loop.run_until_complete(self.connect())

        # sensor beginning
        #self.sense = DashSense(self.device, self.sensor_state)
        #self.sense.start()
        # sensor end
    ...
    def closeDash(self): # validar com erros
        ...
        endingLoop.run_until_complete(self.disconnect())
        time.sleep(0.5)
    ...
    async def command(self, command_name, command_values, sequence = True):
        ...
        if self.device:
            ...
            # print ("escreveu")
            if sequence : await asyncio.sleep(0.5)  # Sleeping just to make sure the response is not missed...
            #await asyncio.sleep(0.5)
    ...
    def eye(self, value):
        ...
        """
        loop = asyncio.new_event_loop()
        loop.run_until_complete(self.command("eye", two_byte_array(value)))
        loop.stop()
